...
class BuildCROWNLib(Task):
   ...
   Compile the CROWN shared libary to be used for all executables with the given configuration
    ...
    def run(self):
        ...
        _install_dir = os.path.abspath(os.path.join(str(self.install_dir), str(self.production_tag)))
        _build_dir = os.path.abspath(os.path.join(str(self.build_dir), str(self.production_tag), "crownlib"))
        _crown_path = os.path.abspath("CROWN")
        _compile_script = os.path.join(
            str(os.path.abspath("processor")), "tasks", "scripts", "compile_crown_lib.sh"
        )
        _local_libfile = os.path.join(_install_dir,"lib", output.basename)
        if os.path.exists(os.path.join(_install_dir, output.basename)):
            console.log(
                "lib already existing in tarball directory {}".format(_install_dir)
            )
            output.parent.touch()
